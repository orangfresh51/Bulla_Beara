...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send(self, code: int, body: bytes, ctype: str) -> None:
        ...
        self.send_header("X-BB-Build", BUILD)
    ...
    def do_GET(self) -> None:
        try:
            path, qs = self._path()
            if path == "/api/health":
                self._json(200, {"ok": True, "app": APP, "build": BUILD, "motto": MOTTO, "ts": time.time(), "tsIso": _utc_iso(time.time())})
                return
            if path == "/api/state":
                self._json(200, self.server.book.state())
                return
            if path == "/api/pulses":
                limit = int(qs.get("limit", "200"))
                self._json(200, self.server.book.pulses(limit))
                return
            if path == "/api/backtest":
                limit = int(qs.get("limit", "800"))
                enter = int(qs.get("enter", "6200"))
                exit_ = int(qs.get("exit", "4800"))
                guard = int(qs.get("volGuard", "8500"))
                ...
                for it in data:
                    pulses.append(
                        Pulse(
                            epoch=int(it["epoch"]),
                            at=float(it["at"]),
                            median_price=float(it["medianPrice"]),
                            bull_bps=int(it["bullScoreBps"]),
                            vol_bps=int(it["volScoreBps"]),
                            mood_bps=int(it["moodScoreBps"]),
                            reveals=int(it["revealsUsed"]),
                            pulse_hash=str(it["pulseHash"]),
                        )
                    )
